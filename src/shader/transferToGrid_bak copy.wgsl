...
@group(0) @binding(3) var<storage, read_write> tempweight: array<vec4<f32>>;
@group(1) @binding(0) var<storage, read_write> tempvelocity: array<vec4<f32>>;
@group(1) @binding(1) var<storage, read_write> mark: array<f32>;
@group(1) @binding(2) var<storage, read_write> output: array<vec4<f32>>;

// fn celltogrid (index: vec3<f32>, resolution: vec3<f32>) -> vec4<u32>{
//     var clampindex = clamp(index, vec3<f32>(0.0), resolution - vec3<f32>(1.0));
//     var gridindex = vec4<u32>(u32(index.x + clampindex.y * (resolution.x + 1.0) + clampindex.z * (resolution.x + 1.0) * (resolution.y + 1.0),
//                               u32(clampindex.x + index.y * (resolution.x + 1.0) + clampindex.z * (resolution.x + 1.0) * (resolution.y + 1.0)),
//                               u32(clampindex.x + clampindex.y * (resolution.x + 1.0) + index.z * (resolution.x + 1.0) * (resolution.y + 1.0)),
//                               u32(clampindex.x + clampindex.y * resolution.x + clampindex.z * resolution.x * resolution.y));
//     return gridindex;
// }
